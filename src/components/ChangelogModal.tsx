...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: '2026-09-02',
    changes: [
      '대량 비행 기록 보유 시 저장 속도 및 검색 성능 개선',
      '기록이 많을 때 연도 필터 변경 시 앱이 멈추던 문제 수정',
      '자동 백업 관련 오류 메시지가 뜨던 문제 수정',
      '파일 불러오기 시 진행 상태를 화면에 표시하도록 개선',
    ],
  },
  {
    version: '1.3.0',
    date: '2026-08-11',
    changes: [
      '데이터 변경 시 자동 백업 기능 추가 (안드로이드는 저작권 및 문의 화면에서 최초 설정 필요, iOS는 자동 적용)',
      '백업에서 복원하기 기능 추가 (백업 파일을 별도 탐색 없이 원클릭으로 불러와 복원)',
      '안드로이드에서 자동 백업 폴더 위치를 화면에 표시하도록 개선',
    ],
  },
  {
    version: '1.2.2',
    date: '2026-07-31',
    changes: [
      '새 기록 입력 시 날짜를 처음 선택해도 바로 반영되지 않던 문제 수정',
      '비행시간 입력 중 "시" 필드를 지우고 다시 입력할 때 자릿수 제한이 풀리던 문제 수정',
      '편조(승무원) 이름 입력 시 자동완성 및 마지막 듀티 자동 입력 기능 추가',
      '안드로이드 뒤로가기 버튼을 눌러도 앱이 바로 종료되던 문제 수정 (이제 이전 화면으로 이동, 메인 화면에서는 종료 확인 팝업 표시)',
    ],
  },
  {
    version: '1.2.1',
